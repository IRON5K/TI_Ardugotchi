// TODO OS INTERVALOS FORAM DIVIDIDOS POR 3 PARA A REALIZAÇÃO DO VÍDEO NEST VERSÃO DO CÓDIGO
// ALGUNS INTERVALOS FICARAM COM '/' POR NÃO DAR DIVISÃO INTEIRA
...
int saude = 0;
unsigned long adicionar[3] = {20000,20000,20000};             // Intervalos associados ao 20 segundos que o Jogador demorar a satisfazer as exigencias
...
void setup(){
  ...
  Intervalos[0] = random(180000/3,300001/3);                           // Gerado um Intervalo entre 1 e 1.67 minutos para o Intervalo de Comer
  Intervalos[1] = random(120000/3,240001/3);                           // Gerado um Intervalo entre 0.67 e 1.33 minutos para o Intervalo de Brincar
  Intervalos[2] = random(540000/3,660001/3);                           // Gerado um Intervalo entre 3 e 3.67 minutos para o Intervalo de Dormir
...
if (saude <= 25){
  ...
  if ((millis() - Ultimo[3]) >= 20000) {                               // Fazemos a média a cada 20 segundos
  ...
  if ((millis() - LastDebounceTime[2]) > Delay) {                      // Verifica se passou tempo suficiente desde a última leitura do botão
    ...
    if (State[2] != Last_State[2] && State[2] == LOW){                 // Verifica se o botão foi pressionado   
      ...
      while ((Ultimo[2] + 100000) > millis()){                         // 300000/3 ms = 1.67 minutos (Tempo que o Ardugotchi dorme)
        desligar_leds();
      }
...
// Função para Determinar novo Intervalo a cada Atividade
void gerar_intervalo(int r){
  if (r == 0) {Intervalos[0] = random(180000/3,300001/3);}          // Intervalo do random é aberto
  if (r == 1) {Intervalos[1] = random(120000/3,240001/3);}
  if (r == 2) {Intervalos[2] = random(540000/3,660001/3);}
}

// Função para Subtrair 5 à Saúde
void bonificacao(int k){
  deduzido[k] = true;
  if ((millis() - Ultimo[k] - Intervalos[k] <= 5000) && (saude != 0) && (deduzido[k] == true)){       // Determina se o Jogador Carregou em até 5 segundos após a notificação
    saude -= 5;
    deduzido[k] = false;    
  }   
}
